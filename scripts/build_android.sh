...
mkdir -p "$source_root" "$temporary_root" "$output_root/release/android"
...
if [ "$build_type" = release ] && [ ! -f "$key_properties" ]; then
    echo "Android release signing is not configured." >&2
    echo "Set GDOX_ANDROID_KEY_PROPERTIES to a complete key.properties file." >&2
    exit 1
fi
...
sha256sum "$output_root/release/android/$output_name"
