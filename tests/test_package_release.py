...
class PackageReleaseTest(unittest.TestCase):
    ...
    def test_windows_release_smoke_uses_headless_background_lifecycle(self) -> None:
        ...
        smoke = (ROOT / "scripts/windows_smoke.ps1").read_text(
            encoding="utf-8"
        )

        self.assertIn(
            "Validate extracted package and Windows background lifecycle",
            workflow,
        )
        self.assertIn('-ArgumentList "--background"', smoke)
        self.assertIn('"GDOXBackgroundHost"', smoke)
        self.assertIn("windows_background_lifecycle=passed", smoke)
        self.assertNotIn("interactive desktop window", smoke)
        self.assertNotIn("WM_CLOSE", smoke)
