...
name is `runtime-` followed by the project version; it deliberately does not
...
Before publishing the 0.2 update, record all of these validation runs in
`STATUS.md`:

- Linux GP63EX70/RF02 launch and clean shutdown with one XGD2 and one XGD3 disc;
- Steam Deck GP63EX70/RF02 launch and clean shutdown with one XGD2 and one XGD3
  disc and its required host helpers installed;
- Steam Deck launch and clean shutdown from a validated owned image;
- Steam Deck Desktop Mode close, restore, and clean quit through a live
  StatusNotifier host;
- Windows native launch and clean shutdown from a validated owned image;
- Windows GP63EX70/RF02 visual confirmation, clean shutdown, and XGD2-to-XGD1
  media swap through the local read-only transport;
- macOS rejection of Xbox 360 image and physical targets, with no Xenia files
  in either architecture's package, plus menu-bar close, restore, Quit, and
  Command-Q cleanup on a native host.
